...
import type { AgentDefinition, AgentCategory, ActivationMode } from '@/lib/agents/types'
import { AgentState } from '@/lib/agents/types'
import type { AgentRequirements, GeneratedAgent } from './types'
import { VibeCodingError } from './types'
import { createAgentId } from '@/types/conversation'
...
function generateAgentId(requirements: AgentRequirements): string {
  ...
  return `${baseName}-${timestamp}-${random}` as any
}
