...
export class HealthMonitor {
  ...
  private async collectPluginMetrics(timestamp: number): Promise<void> {
    try {
      const storage = getPluginStore();
      const manifests = await storage.getAllManifests();
      const states = await storage.getAllStates();

      const enabled = states.filter((s) => s.enabled).length;
      const disabled = states.filter((s) => !s.enabled).length;
      const errors = states.filter((s) => s.state === 'error').length;

      const healthScore = manifests.length > 0
        ? ((enabled - errors) / manifests.length) * 100
        : 100;
      ...
      this.updateMetric({
        ...
        unit: 'count',
        timestamp,
        status: errors > 0 ? HealthStatus.WARNING : HealthStatus.HEALTHY,
      });
      ...
      this.updateMetric({
        ...
        unit: 'score',
        timestamp,
        status: this.getHealthStatus('plugin-health-score', healthScore, true),
  ...
  private async collectAgentMetrics(timestamp: number): Promise<void> {
    try {
      ...
      const active = allAgents.filter((a) => a.initialState.status === 'running').length;
      const idle = allAgents.filter((a) => a.initialState.status === 'idle').length;
      const errors = allAgents.filter((a) => a.initialState.status === 'error').length;

      const healthScore = allAgents.length > 0
        ? ((active + idle) / allAgents.length) * 100
        : 100;
      ...
      this.updateMetric({
        ...
        unit: 'score',
        timestamp,
        status: this.getHealthStatus('agent-health-score', healthScore, true),
