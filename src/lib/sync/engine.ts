...
import { getConversation, listConversations, getMessages } from '@/lib/storage/conversation-store'
...
export class SyncEngine {
  ...
  private async collectLocalDeltas(): Promise<DataDelta[]> {
    ...
    // Collect message changes (recent ones only, since last sync)
    // In production, track which messages need syncing
    ...
    for (const delta of deltas) {
      try {
        switch (delta.collection) {
          case 'conversations':
            await this.applyConversationDelta(delta)
            break
  ...
  private async applyConversationDelta(delta: DataDelta): Promise<void> {
    const conversation = delta.data as any

    const existing = await getConversation(conversation.id)
    if (existing) {
      // Merge updates
      // In production, implement smart merge
    } else {
      // Create new
      // Would call createConversation
    }
  }
  ...
  private async createBackup(): Promise<void> {
    ...
    // In production, export critical data
