...
import { JSONConverter } from '../export/converters/json'
...
export class ImportManager {
  ...
  async importFromJSON(jsonData: any): Promise<ImportPreview> {
    ...
    const validation = this.jsonConverter.validateJSON(jsonData)

    if (!validation.valid) {
      throw new Error(`Invalid JSON: ${validation.errors.join(', ')}`)
    }

    // Convert PersonalLog format
   ...
   */
  private detectSourceType(content: string, filename: string): ImportSourceType {
    // Try ChatGPT format
  ...
  private async importItem(data: any): Promise<void> {
    // Create conversation
    const conversation = await createConversation(data.title, data.type)
   ...
   */
  private async applyResolution(conflict: ImportConflict, resolved: any): Promise<void> {
