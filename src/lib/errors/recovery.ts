...
export class WasmRecoveryStrategy {
   ...
   */
  async getFallback(moduleName: string): Promise<any> {
    // Check cache first
    ...
    try {
      ...
      log(error, { component: 'WasmRecoveryStrategy', operation: 'loadJsFallback' });
      throw error;
    }
  }
   ...
   */
  private async loadJsFallback(moduleName: string): Promise<any> {
...
export class StorageRecoveryStrategy {
  ...
  async checkStorageQuota(): Promise<{
    ...
    if ('storage' in navigator && 'estimate' in navigator.storage) {
      try {
        ...
        log(error, { component: 'StorageRecoveryStrategy', operation: 'checkStorageQuota' });
      }
    }
   ...
   */
  async recoverSpace(targetBytes: number): Promise<number> {
    let recovered = 0;
  ...
  private async clearOldCache(): Promise<number> {
    if ('caches' in window) {
      try {
        ...
        log(error, { component: 'StorageRecoveryStrategy', operation: 'clearOldCache' });
      }
    }
  ...
  private async compactOldConversations(): Promise<number> {
    try {
      ...
      log(error, { component: 'StorageRecoveryStrategy', operation: 'compactOldConversations' });
      return 0;
    }
  }
  ...
  private async clearTemporaryData(): Promise<number> {
    try {
      ...
      log(error, { component: 'StorageRecoveryStrategy', operation: 'clearTemporaryData' });
    }
   ...
   */
  private async listOldConversations(daysOld: number): Promise<any[]> {
  ...
  async requestQuota(requestedBytes: number): Promise<boolean {
    if ('storage' in navigator && 'requestPersistent' in navigator.storage) {
      try {
        ...
        log(error, { component: 'StorageRecoveryStrategy', operation: 'requestQuota' });
      }
    }
  ...
  async getCached(url: string): Promise<Response | null> {
    if ('caches' in window) {
      try {
        ...
        log(error, { component: 'NetworkRecoveryStrategy', operation: 'getCached' });
      }
    }
  ...
  async cacheResponse(url: string, response: Response): Promise<void> {
    if ('caches' in window && response.ok) {
      try {
        ...
        log(error, { component: 'NetworkRecoveryStrategy', operation: 'cacheResponse' });
      }
    }
  }
}
...
export function initializeRecoveryStrategies(): void {
  // WASM recovery
  registerRecoveryActions('wasm-fallback', (error) => [
    {
