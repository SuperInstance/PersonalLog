 ...
 *
 * Single point of truth for error handling.
...
export class ErrorHandler {
  ...
  private recoveryCallbacks: Map<ErrorCategory, Set<(error: CentralError) => RecoveryAction[]>> = new Map();
  ...
  private logError(error: ErrorRecord): void {
    ...
    // Main error message
    console.error(
      `%c${emoji} Central Error Manager [${error.category.toUpperCase()}]`,
      style,
      error.userMessage
    );
  ...
  registerRecoveryActions(
    category: ErrorCategory,
    callback: (error: CentralError) => RecoveryAction[]
  ): () => void {
