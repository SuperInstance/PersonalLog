...
export class ConflictResolver {
  private dataProvider?: ConflictDataProvider

  constructor(dataProvider?: ConflictDataProvider) {
  ...
  async detectItemConflicts(item: ImportItem): Promise<ImportConflict[]> {
    ...
    if (!this.dataProvider) {
      // No data provider, assume no conflicts
      return conflicts
    }

    // Check for duplicate ID
    const existingById = await this.dataProvider.getItemById(item.sourceId)
    if (existingById) {
      conflicts.push({
    ...
    const allItems = await this.dataProvider.getAllItems()
    const withSameTitle = allItems.find(i => i.title === item.title && i.id !== item.sourceId)
    if (withSameTitle) {
      conflicts.push({
        ...
        description: `An item with title "${item.title}" already exists`,
...
/**
 * Data provider interface for conflict detection
 */
