 ...
 * Agent Registry
 *
 * Central registry for managing AI agent lifecycles.
 * Handles registration, availability checking, and agent lifecycle management.
 *
 * @example
 * ```typescript
 ...
 * const availability = await agentRegistry.checkAvailability('my-agent-v1', hardwareProfile);
 *
...
export class AgentRegistry {
  ...
  async checkAvailability(agentId: string, hardwareProfile: HardwareProfile): Promise<AgentAvailabilityResult> {
    ...
    if (agent.requirements?.hardware) {
      ...
      if (hw.minJEPAScore !== undefined) {
        const jepaScore = this.calculateJEPAScore(hardwareProfile);
        if (jepaScore < hw.minJEPAScore) {
          missingHardware.push(
            `JEPA score ${jepaScore} below minimum ${hw.minJEPAScore}`
          );
        }
      }
    ...
    // Note: Feature flag checking is not included in standalone version
    // This would need to be integrated with a feature flag system
  ...
  /**
   * Calculate JEPA score from hardware profile
   *
   * @param hardwareProfile - Hardware profile
   * @returns JEPA score (0-100)
   */
  private calculateJEPAScore(hardwareProfile: HardwareProfile): number {
    let score = 0;

    // GPU contribution
    if (hardwareProfile.gpu.available) {
      score += 40;
      if (hardwareProfile.gpu.webgpu.supported) {
        score += 20;
      }
    }
    ...
    // RAM contribution
    const ram = hardwareProfile.memory.totalGB;
    score += Math.min(ram * 2, 15);
