  ...
  private async connectDropbox(): Promise<void> {
    ...
    if (!response.ok) {
      throw new NetworkError('Dropbox connection failed', {
        technicalDetails: `Dropbox API error: ${response.statusText} (status: ${response.status})`
      })
    }
  }
  ...
  private async connectGoogleDrive(): Promise<void> {
    ...
    if (!response.ok) {
      throw new NetworkError('Google Drive connection failed', {
        technicalDetails: `Google Drive API error: ${response.statusText} (status: ${response.status})`
      })
    }
  }
  ...
  private async connectOneDrive(): Promise<void> {
    ...
    if (!response.ok) {
      throw new NetworkError('OneDrive connection failed', {
        technicalDetails: `OneDrive API error: ${response.statusText} (status: ${response.status})`
      })
    }
  }
