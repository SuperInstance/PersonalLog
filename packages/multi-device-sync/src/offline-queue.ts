...
import { DataDelta, QueuedChange, OfflineQueueStats } from './types'
...
export class OfflineQueue {
  ...
  async processQueue(syncFn?: (change: QueuedChange) => Promise<boolean>): Promise<void> {
    ...
    try {
      ...
      // Log results
      console.log(`[OfflineQueue] Processed ${pendingChanges.length} changes`)
    } finally {
