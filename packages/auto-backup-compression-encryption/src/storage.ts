...
import { StorageError, QuotaError } from './errors'

// ============================================================================
// DATABASE CONSTANTS
// ============================================================================

const DB_NAME = 'AutoBackupSystem'
const DB_VERSION = 1
 ...
 */
export async function saveBackup<T>(backup: Backup<T>): Promise<void> {
  try {
 ...
 */
export async function getBackup<T>(id: string): Promise<Backup<T> | null> {
  try {
    const database = await getDB()
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = async () => {
        ...
        const backup = await decompressBackup<T>(request.result)
        resolve(backup)
      }
 ...
 */
export async function listBackups<T>(options?: {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = async (event) => {
        ...
        if (cursor) {
          const backup = cursor.value as Backup<T>
